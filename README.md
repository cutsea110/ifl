...
You should specify the version 0.3.34, because the latest version is 0.3.33.

```bash
$ docker buildx build --rm --load -t cutsea110/ifl:0.3.34 .
```
...
```bash
$ docker run -v ${PWD}/examples:/work -it --rm cutsea110/ifl:0.3.34 -v -c pgmark4 /work/testProg160.ifl
```

or try this.

```bash
$ docker run -v ${PWD}/examples:/work -it --rm cutsea110/ifl:0.3.34 -t 1000 -L -v -c timark6 /work/testProg159.ifl
```

Further more, just only on the TMark6, You can use -p option which profile your code.

```bash
$ docker run -v ${PWD}/examples:/work -it --rm cutsea110/ifl:0.3.34 -t 1000 -L -v -c timark6 -p /work/testProg159.ifl
```
...
```bash
$ docker run -v ${PWD}/examples:/work -it --rm cutsea110/ifl:0.3.34 -w -c pgmark4 /work/testProg160.ifl
```
...
```bash
$ docker login
$ docker push cutsea110/ifl:0.3.34
```
